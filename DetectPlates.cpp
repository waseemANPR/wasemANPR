...
std::vector<PossiblePlate> detectPlatesInScene(cv::Mat &imgOriginalScene) {
	...
	/*start from here*/	std::vector<std::vector<PossibleChar> > vectorOfVectorsOfMatchingCharsInScene = findVectorOfVectorsOfMatchingChars(vectorOfPossibleCharsInScene);
	...
	for (unsigned int i = 0; i < vectorOfPossiblePlates.size(); i++) {
		...
		for (int j = 0; j < 4; j++) {
			cv::line(imgContours, p2fRectPoints[j], p2fRectPoints[(j + 1) % 4], SCALAR_RED, 10);
		}
		cv::imshow("4a", imgContours);
...
PossiblePlate extractPlate(cv::Mat &imgOriginal, std::vector<PossibleChar> &vectorOfMatchingChars) {
	...
	//std::cout<<"vectorOfMatchingChars[0].intCenterX" << vectorOfMatchingChars[vectorOfMatchingChars.size() - 1].intCenterX << std::endl;
	...
	std::cout << "dblCorrectionAngleInDeg" << dblCorrectionAngleInDeg << std::endl;
